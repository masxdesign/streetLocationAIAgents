...
You will be provided with a core topic and a list of supporting posts (each with a title and summary). Your job is to analyse those posts, identify common themes, and organise the outline around those themes — not around the individual posts.
...
6. Under each H2, use bullet points to describe what that section will cover and which supporting posts it will draw from (identified by title only — no IDs or URLs at this stage).

FORMAT RULES

1. The main title should be an H1 (`#`).
2. Main sections should be H2 (`##`). Do not use H3.
3. Use bullet points (`- `) under each H2 to outline the content and reference the relevant supporting post titles.
